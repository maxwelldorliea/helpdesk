<template>
  <span>
    ...
    <div v-if="isEmpty(items.data)" class="flex grow select-none items-center justify-center text-base text-gray-700">
    ...
    <div v-else class="space-y-3 overflow-y-scroll p-5">
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <SingleString v-for="i in items.data.filter((i) => !i.is_chart)" :key="i.title" :title="i.title"
          :value="i.data" />
      </div>
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div v-for="i in items.data.filter((i) => i.is_chart)" :key="i.title" class="h-64 rounded border p-2">
          <PieChart v-if="i.is_chart && i.chart_type === 'Pie'" :title="i.title" :data="i.data" />
          <LineChart v-else-if="i.is_chart && i.chart_type === 'Line'" :title="i.title" :data="i.data" />
        </div>
      </div>
    </div>
  </span>
